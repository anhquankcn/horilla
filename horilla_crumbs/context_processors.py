...
def breadcrumbs(request):
    ...
    if "breadcrumbs" not in request.session:
        request.session["breadcrumbs"] = [
            {"url": base_url, "name": "Trang chủ", "found": True}
        ]
    ...
    try:
        ...
        if len(parts) == 0:
            ...
            breadcrumbs.append({"url": base_url, "name": "Trang chủ", "found": True})
...
urlpatterns.append(path("eoffice/", lambda request: redirect("eoffice-board")))
