...
| Nguồn | Sinh tự động từ codebase + tài liệu CLAUDE.md |

> Tài liệu này mô tả **yêu cầu** của hệ thống. Thiết kế kỹ thuật xem [TDD.md](./TDD.md).
...
Hệ thống chạy như **monolith Django + PWA tách rời**, đứng sau nginx + Cloudflare Tunnel, xác thực tập trung qua Keycloak. Dữ liệu lưu PostgreSQL, cache/queue qua Redis. Triển khai bằng Docker Compose trên 2 môi trường (prod, stage). Chi tiết kiến trúc: [TDD.md](./TDD.md).
...
- FR-3.2 Kiểm tra geofence theo văn phòng gần nhất; ngoài vùng → đánh dấu `geo_valid=false` (KHÔNG chặn chấm), yêu cầu chọn lý do làm từ xa.
...
- FR-4.7 Phép bù: manager đề xuất → C&B duyệt (HNHCompensatoryProposal).
- FR-4.8 Cấp phát phép (allocation), carryforward, reset theo năm; export Excel; tổng quan phép.
...
- FR-6.3 Đào tạo (Training), Đánh giá hiệu suất (PMS: mục tiêu/KPI), Thăng chức (Promotion multi-level), Offboarding.
...
| NFR-8 | Vận hành | Deploy qua git pull + docker build trên prod/stage; migration test stage trước prod; nhật ký chẩn đoán grep được |
...
| **Microsoft 365** | Federated identity (login bằng email công ty) | KC Identity Provider |
...
Các yêu cầu được hiện thực trong các module Django (`employee`, `attendance`, `leave`, `payroll`, `base`, `horilla_api`, `notifications`...) và PWA (`pwa/frontend`). Chi tiết ánh xạ requirement → thiết kế/endpoint/model xem [TDD.md](./TDD.md).

Ưu tiên: **P0** (chấm công, nghỉ phép, lương, SSO) đang vận hành production. **P1** (chi phí, tài sản, PMS, đào tạo, thăng chức) đã có. **P2** (gamification WC2026, tích hợp app ngoài) tùy chọn theo feature flag.
