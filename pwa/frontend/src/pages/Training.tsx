...
type Tab = 'overview' | 'my_courses' | 'catalog' | 'team'

const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: 'overview', label: 'Tổng quan', icon: 'bar-chart' },
  { id: 'my_courses', label: 'Khóa học', icon: 'book' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: HNH.ink3 }}>
            {data.completed}/{data.total_enrollments} khóa học
          </div>
        </div>
      </div>
      ...
      {data.mandatory_pending > 0 && (
        <div style={{
          background: HNH.red50, borderRadius: 16, padding: 16,
          border: `1px solid ${HNH.red}22`,
          ...
          <div style={{ marginLeft: 'auto' }}><Icon name="chev-r" size={14} color={HNH.ink3} /></div>
        </div>
      )}
      ...
      {data.is_manager && data.team_enrollments > 0 && (
        ...
        }} onClick={() => onGoTab('team')}>
          <div style={{ fontSize: 13, fontWeight: 600, opacity: 0.8, marginBottom: 8 }}>
            Đào tạo Team
          </div>
          <div style={{ display: 'flex', gap: 24 }}>
            <div>
              <div style={{ fontSize: 28, fontWeight: 700 }}>{data.team_enrollments}</div>
              <div style={{ fontSize: 12, opacity: 0.7 }}>Đăng ký</div>
            </div>
            <div>
              <div style={{ fontSize: 28, fontWeight: 700 }}>{data.team_completed}</div>
              <div style={{ fontSize: 12, opacity: 0.7 }}>Hoàn thành</div>
            </div>
            <div>
  ...
  return (
    <div style={{
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 16, cursor: 'pointer' }}>
        ...
        {/* Meta row */}
        <div style={{
          display: 'flex', alignItems: 'center', gap: 10, marginTop: 8, fontSize: 11, color: HNH.ink3,
        }}>
          {(course.instructor_employee || course.instructor) && (
            <span>GV: {course.instructor_employee || course.instructor}</span>
          )}
          {course.start_date && <span>{formatDate(course.start_date)} → {formatDate(course.end_date)}</span>}
          {course.location && <span>{course.location}</span>}
          <div style={{
            marginLeft: 'auto', transform: expanded ? 'rotate(180deg)' : 'none',
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${HNH.line}`, padding: 16 }}>
          {course.description && (
            <div style={{ fontSize: 13, color: HNH.ink2, marginBottom: 12, lineHeight: 1.5 }}>
              {course.description}
            </div>
          )}
          ...
          <div style={{
            ...
            <div style={{
              background: HNH.cream, borderRadius: 8, padding: '8px 12px',
            }}>
            ...
            {course.max_participants > 0 && (
              <div style={{
                ...
                <div style={{ color: HNH.ink3 }}>Tối đa</div>
                <div style={{ fontWeight: 600, color: HNH.ink }}>{course.max_participants}</div>
              </div>
            )}
            {enrollment?.score != null && (
              <div style={{
                ...
                <div style={{ color: HNH.ink3 }}>Điểm số</div>
                <div style={{ fontWeight: 600, color: HNH.ink }}>{enrollment.score}</div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {enrollment && enrollment.status === 'enrolled' && onUpdateStatus && (
              <button
                onClick={() => onUpdateStatus(enrollment.id, 'in_progress')}
                style={{
                  padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer',
            ...
            {enrollment && enrollment.status === 'in_progress' && onUpdateStatus && (
              <button
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 16, cursor: 'pointer' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {member.avatar ? (
            ...
            <div style={{
              width: 40, height: 40, borderRadius: 12,
              background: HNH.navy + '18', display: 'flex', alignItems: 'center', justifyContent: 'center',
...
export function TrainingPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: HNH.cream, display: 'flex', flexDirection: 'column' }}>
      ...
      {tab === 'catalog' && data?.categories && data.categories.length > 0 && (
        <div style={{
          display: 'flex', gap: 4, padding: '4px 16px',
          overflowX: 'auto', WebkitOverflowScrolling: 'touch',
          maxWidth: mx, margin: mx ? '0 auto' : undefined, width: '100%',
      ...
      <div style={{
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {tab === 'overview' && (
              <OverviewTab data={data as OverviewData} onGoTab={t => { setData(null); setTab(t) }} />
            )}
            ...
            {tab === 'catalog' && (data.courses || []).length === 0 && (
              <EmptyState icon="graduation" text="Chưa có khóa học nào" sub="Quản trị viên tạo khóa tại Admin" />
            )}
            {tab === 'catalog' && (data.courses || []).map((c: Course) => (
              <CourseCard key={c.id} course={c}
                onEnroll={handleEnroll} onUpdateStatus={handleUpdateStatus} onCancel={handleCancel} />
            ))}

            {tab === 'team' && !data.is_manager && (
              <EmptyState icon="users" text="Chỉ quản lý mới xem được" />
            )}
